                ...
                >
                    <motion.div variants={heroTextReveal} className="mb-8">
                        <span className="inline-flex items-center gap-3 px-6 py-2.5 rounded-full bg-brand-glass border border-brand-accent/30 text-brand-accent text-sm font-mono tracking-[0.2em] shadow-neon-cyan backdrop-blur-md">
                            ...
                            <span className="uppercase">Licensed Swiss Tech Company</span>
                        </span>
                    </motion.div>
                    ...
                    <motion.p variants={heroTextReveal} className="text-xl md:text-2xl text-brand-muted max-w-3xl mx-auto leading-relaxed font-light">
                        We architect enterprise-grade digital systems and bespoke software solutions with <span className="text-white font-semibold">Swiss precision</span>. Every system we build is engineered for performance, security, and global scale.
                    </motion.p>
                </motion.div>
            </div>
                        ...
                        <div className={!isEven ? 'lg:order-2' : ''}>
                            ...
                            <div className="flex flex-wrap gap-4">
                                <Link href={`/services/${service.id}`}>
                                    <Button variant="primary" size="lg" icon={<ArrowRight size={20} />} className="shadow-neon-cyan">
                                        Exploration Hub
                                    </Button>
                                </Link>
                                <Link href={`${WHATSAPP_URL}?text=Hi, I'm interested in ${service.title}`} target="_blank">
                                    <Button variant="outline" size="lg" icon={<MessageCircle size={20} />} className="border-white/10 hover:border-brand-accent/30">
                                        Expert Consult
                                    </Button>
                                </Link>
                            </div>
                    ...
                    <SectionReveal direction={isEven ? 'right' : 'left'}>
                        <div className={`relative ${!isEven ? 'lg:order-1' : ''}`}>
                            ...
                            <div className="glass-card p-10 relative bg-brand-dark/50 border-white/10 backdrop-blur-2xl overflow-hidden group">
                                <div className="absolute top-0 right-0 w-48 h-48 bg-brand-accent/5 rounded-full blur-[80px] -mr-24 -mt-24 group-hover:bg-brand-accent/10 transition-colors" />
                                ...
                                <ul className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {service.features.map((feature, i) => (
                                        ...
                                        >
                                            <div className="w-2 h-2 rounded-full bg-brand-accent shadow-neon-cyan group-hover/item:scale-150 transition-transform" />
                                ...
                                <div className="mt-12 pt-8 border-t border-white/5 flex items-center justify-between">
                                    ...
                                    <span className="text-[10px] font-mono uppercase tracking-widest text-brand-muted/40">Verified Architecture</span>
                                </div>
                            </div>
                        </div>
                    </SectionReveal>
...
function RoadmapSection() {
    const steps = [
        { title: 'Discovery & Audit', desc: 'In-depth analysis of your business requirements and technical ecosystem.', icon: <LucideIcons.Search size={24} /> },
        { title: 'Strategic Architecture', desc: 'Designing a scalable system blueprint with Swiss engineering precision.', icon: <LucideIcons.Layout size={24} /> },
        { title: 'Agile Development', desc: 'Iterative coding process with real-time feedback and transparent milestones.', icon: <LucideIcons.Cpu size={24} /> },
        { title: 'QA & Deployment', desc: 'Rigorous testing protocols followed by seamless global delivery.', icon: <LucideIcons.ShieldCheck size={24} /> },
    ];
    ...
    return (
        <section className="section-padding relative overflow-hidden bg-brand-primary border-t border-white/5">
            <div className="absolute inset-0 tech-grid opacity-5" />
            <div className="section-container relative z-10">
                <SectionHeading badge="The Process" title="Technical Roadmap" />
                ...
                <div className="section-container relative z-10">
                    <div className="glass-card p-12 md:p-20 text-center max-w-5xl mx-auto border-brand-accent/20 shadow-neon-cyan overflow-hidden relative">
                        ...
                        <div className="relative z-10">
                            <span className="text-brand-accent font-mono text-xs tracking-[0.5em] uppercase mb-6 block">Initiate Your Project</span>
                            <h2 className="text-4xl md:text-6xl font-display font-black text-white mb-8 tracking-tighter">
                                Ready to Architect <br />
                                <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-accent via-white to-brand-secondary">Something Amazing?</span>
                            </h2>
                            <p className="text-brand-muted text-xl max-w-2xl mx-auto mb-12 font-light leading-relaxed">
                                Every enterprise begins with a single line of strategic code. Let&apos;s discuss your vision and build a solution that lasts.
                            </p>
                            <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
                                <Link href="/contact" className="w-full sm:w-auto">
                                    <Button size="xl" variant="primary" icon={<ArrowRight size={24} />} className="w-full sm:min-w-[240px] shadow-neon-cyan">
                                        Launch Solution
                                    </Button>
                                </Link>
                                <Link href={WHATSAPP_URL} target="_blank" className="w-full sm:w-auto">
                                    <Button size="xl" variant="outline" icon={<MessageCircle size={24} />} className="w-full sm:min-w-[240px] border-white/10 hover:border-brand-accent/30">
                                        WhatsApp Intel
                                    </Button>
                                </Link>
                            </div>
