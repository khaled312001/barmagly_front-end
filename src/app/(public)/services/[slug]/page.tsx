...
import {
    Code2, Smartphone, Palette, ShoppingCart, TrendingUp,
    ArrowRight, CheckCircle2, Shield, Zap, Globe, MessageCircle,
    ArrowLeft, ChevronRight, Cpu, Database, Cloud, Terminal
} from 'lucide-react';
...
export default function ServiceDetailPage() {
    ...
    useEffect(() => {
        const fetchService = async () => {
            console.log(`[ServiceDetailPage] Fetching data for slug: ${slug}`);
            try {
                const { data } = await publicApi.getService(slug as string);
                console.log('[ServiceDetailPage] Service data received:', data);
                setService(data);
            } catch (error: any) {
                console.error('[ServiceDetailPage] Failed to fetch service:', error);
                if (error.code === 'ERR_NETWORK') {
                    console.error('[ServiceDetailPage] Backend might be down or unreachable at:', publicApi.getServices().toString());
                }
            } finally {
                setLoading(false);
            }
            ...
            <div className="min-h-screen flex items-center justify-center bg-brand-primary">
                <div className="flex flex-col items-center gap-6">
                    <div className="relative w-20 h-20">
                        ...
                        <div className="absolute inset-0 border-4 border-t-brand-accent rounded-full animate-spin" />
                        <motion.div
                    ...
                    <p className="text-brand-accent font-mono text-sm tracking-widest animate-pulse">INITIALIZING SYSTEM...</p>
                </div>
            </div>
        );
    }
