...
export function TopHeader({ lang, dict, scrolled = false }: { lang: string, dict: any, scrolled?: boolean }) {
    ...
    return (
        ...
        >
            <div className="section-container !py-0 flex items-center justify-between gap-6 text-xs font-bold">
                <div className="flex items-center gap-4 min-w-0">
                    <a
                        href={`tel:${OFFICE_PHONE.replace(/\s/g, '')}`}
                        className="flex items-center gap-2 text-brand-muted hover:text-brand-accent transition-colors duration-300 whitespace-nowrap"
                    >
                        <Phone size={14} className="text-brand-accent" />
                        <span dir="ltr" className="tracking-wider">{OFFICE_PHONE}</span>
                    </a>
                    ...
                    <a
                        ...
                        className="flex items-center gap-2 text-brand-muted hover:text-[#25D366] transition-colors duration-300 whitespace-nowrap"
                    >
                        ...
                        <span dir="ltr" className="tracking-wider">{WHATSAPP_PHONE}</span>
                    </a>
                </div>

                <div className="flex items-center gap-4 shrink-0">
                    <span className="text-brand-muted/50 uppercase text-[10px] tracking-wider">{dict?.nav?.followUs || 'Follow Us'}</span>
